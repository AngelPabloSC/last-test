...
import AdminRequestViewDialog from '@/components/dialogs/AdminRequestViewDialog';
import AdminReviewDetailDialog from '@/components/dialogs/AdminReviewDetailDialog';

export default function AdminSidebarLayout() {
  return (
    <NotificationProvider>
      <AdminSidebarLayoutContent />
    </NotificationProvider>
  );
}

function AdminSidebarLayoutContent() {
  ...
  const { menuItems: baseMenuItems } = filterRouter(user?.rol);

  // Inject dynamic badge into menuItems
  const menuItems = useMemo(() => {
    return baseMenuItems.map(item => {
      if (item.id === 'solicitudes') {
        return { ...item, badge: newCount };
      }
      if (item.id === 'reviews') {
  ...
  }, [baseMenuItems, newCount, pendingReviewsCount]);
  ...
  return (
    <>
      ...
      {/* Global Request Details Dialog */}
      <AdminRequestViewDialog
        open={isDetailOpen}
