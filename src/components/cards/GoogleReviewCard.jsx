...
export default function GoogleReviewCard({ review }) {
  return (
    <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2, borderRadius: '16px', borderLeft: `3px solid ${review.avatarColor}`, bgcolor: '#1A1A1A', p: { xs: 3, md: 4 } }}>
      ...
      <ReviewStarRating rating={review.rating} size={14} color="#FBBC05" />
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid #2A2A2A', pt: 2, mt: 1 }}>
        <Box 
          component={review.url && review.url !== '#' ? "a" : "div"}
          href={review.url && review.url !== '#' ? review.url : undefined}
          target="_blank"
          rel="noopener noreferrer"
          sx={{ display: 'flex', alignItems: 'center', gap: 0.5, textDecoration: 'none', cursor: review.url && review.url !== '#' ? 'pointer' : 'default', transition: 'opacity 0.2s', '&:hover': { opacity: review.url && review.url !== '#' ? 0.7 : 1 } }}
        >
          <Typography sx={{ fontSize: 13, fontWeight: 900, color: '#4285F4' }}>G</Typography>
          <Typography sx={{ fontSize: 11, color: '#666', transition: 'color 0.2s', '&:hover': { color: review.url && review.url !== '#' ? '#4285F4' : '#666' } }}>Posted on Google</Typography>
        </Box>
