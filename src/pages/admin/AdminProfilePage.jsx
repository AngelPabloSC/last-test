...
export default function AdminProfilePage() {
  ...
  const { preferences, loading: notifLoading, togglingId, togglePreference } = useNotificationPreferences();
        ...
        <Grid container spacing={2.5}>
          ...
          <Grid size={{ xs: 12, md: 7.5, lg: 8.2 }} sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            ...
            <ProfileCard title="Recent Activity">
              <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                {activitiesLoading ? (
                  <Typography sx={{ color: '#6B7280', fontSize: 13, py: 2, textAlign: 'center' }}>
          ...
          <Grid size={{ xs: 12, md: 4.5, lg: 3.8 }} sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            ...
            <ProfileCard title="Notification Preferences">
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                {notifLoading ? (
                  <Typography sx={{ color: '#6B7280', fontSize: 13 }}>Loading preferences...</Typography>
                ) : preferences.length === 0 ? (
                  <Typography sx={{ color: '#6B7280', fontSize: 13 }}>No notification preferences available.</Typography>
                ) : preferences.map((pref) => (
                  <Box key={pref.id} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <Box>
                      <Typography sx={{ fontSize: 14, fontWeight: 600 }}>{pref.label}</Typography>
                      <Typography sx={{ color: '#6B7280', fontSize: 12 }}>
                         {pref.code === 'contacts' ? 'Instant email and mobile alert' : 'Instant email alert'}
                      </Typography>
                    </Box>
                    <Switch 
                      checked={pref.enabled} 
                      disabled={togglingId === pref.id}
                      onChange={() => togglePreference(pref.id, pref.enabled)}
                      sx={{
                        '& .MuiSwitch-switchBase.Mui-checked': { color: 'primary.main' },
                        '& .MuiSwitch-switchBase.Mui-checked + .MuiSwitch-track': { bgcolor: 'primary.main' },
                        opacity: togglingId === pref.id ? 0.5 : 1
                      }}
                    />
                  </Box>
                ))}
