...
export default function GalleryProjectForm({ initialData = {}, onSubmit, onStatusChange, isSubmitting }) {
  const {
    isEdit,
    control,
    handleSubmit,
  ...
  // Map form values to project object for the preview dialog
  const previewProject = {
    ...formValues,
    serviceType: serviceTypes.find(t => t.id === formValues.serviceTypeId),
    img: formValues.images?.[formValues.coverImageIndex]?.url || formValues.images?.[formValues.coverImageIndex],
    techDetails: {
        duration: 'N/A',
        teamSize: 'N/A',
        cost: 'N/A'
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
          ...
          <Paper sx={{ p: 4, bgcolor: '#0A0A0A', borderRadius: '12px', border: '1px solid #1F1F1F', mb: 3 }}>
            <Typography variant="subtitle2" sx={{ 
              ...
              <Button 
                size="small" 
                variant="outlined" 
                onClick={previewDialog.handleOpenDialog}
                sx={{ 
                  fontSize: '10px', 
                  py: 0.5, 
                  px: 1.5, 
                  ...
                  borderColor: 'primary.main',
                  color: 'primary.main',
                  '&:hover': { bgcolor: 'rgba(212, 175, 55, 0.05)', borderColor: 'primary.main' }
                }}
              >
                Full Preview
              </Button>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText sx={{ color: 'rgba(255,255,255,0.7)', mb: 2 }}>
            {isEdit 
              ? 'Changes will be applied and visible immediately on the public site gallery once updated.' 
      ...
      >
        ...
        <DialogContent>
          <DialogContentText sx={{ color: 'rgba(255,255,255,0.7)', mb: 2 }}>
            Are you sure you want to {pendingStatus === 'Visible' ? 'publish' : 'hide'} this project? This change will be immediately reflected on the server.
          </DialogContentText>
        </DialogContent>
        <DialogActions sx={{ p: 3, pt: 0 }}>
          <Button 
          ...
          <Button 
            ...
            variant="contained" 
            sx={{ fontWeight: 800, px: 3, bgcolor: pendingStatus === 'Visible' ? 'primary.main' : 'warning.main', color: '#000' }}
          >
            Yes, Change Status
          </Button>
        </DialogActions>
      ...
      <Dialog
        ...
        onClose={deleteImageDialog.handleCloseDialog}
        PaperProps={{
          sx: { bgcolor: '#0A0A0A', border: '1px solid #333', borderRadius: '12px', minWidth: '400px' }
        }}
      >
        <DialogTitle sx={{ color: 'white', fontWeight: 700, pt: 3 }}>Delete Image?</DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ color: 'rgba(255,255,255,0.7)', mb: 2 }}>
            Are you sure you want to remove this image from the project gallery? This action is permanent.
          </DialogContentText>
        </DialogContent>
        <DialogActions sx={{ p: 3, pt: 0 }}>
          <Button onClick={deleteImageDialog.handleCloseDialog} sx={{ fontWeight: 700, color: 'rgba(255,255,255,0.4)' }}>Cancel</Button>
          <Button 
            onClick={confirmImageDelete} 
            variant="contained" 
            disabled={isDeletingImage}
            sx={{ fontWeight: 800, px: 3, bgcolor: '#F44336', color: 'white' }}
          >
      ...
      {/* FULL LIVE PREVIEW DIALOG */}
      <GalleryDetailDialog 
        open={previewDialog.isOpen}
        onClose={previewDialog.handleCloseDialog}
