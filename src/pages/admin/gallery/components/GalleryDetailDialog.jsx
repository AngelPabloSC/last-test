...
export default function GalleryDetailDialog({ open, onClose, project }) {
  ...
  const metadata = [
    ...
    { label: 'Category', value: project.category || project.serviceType?.name || 'Category', icon: <SquareFootIcon fontSize="small" /> },
  ];
    ...
    >
      <Box sx={{ position: 'relative' }}>
        ...
        <Box sx={{ position: 'relative', height: 300, overflow: 'hidden' }}>
          <Box
            component="img"
            src={project.img || (project.images && project.images[0]?.url) || project.images?.[0] || 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=1440&q=80'}
          ...
          <Box sx={{ position: 'absolute', bottom: 0, left: 0, right: 0, p: 4, background: 'linear-gradient(to top, #0A0A0A 0%, transparent 100%)' }}>
            <Chip 
              label={project.category || project.serviceType?.name || 'Project'} 
