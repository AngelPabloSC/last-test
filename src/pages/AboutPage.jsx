...
function StatsBar({ totalProjects = 0, averageRating = "4.8", ratingCount = "500+" }) {
  const theme = useTheme();
  const gold = theme.palette.primary.main;
  const darkText = theme.palette.primary.contrastText;
  
  // Animate the dynamically fetched total with scroll detection
  ...
  const stats = [
    ...
    { number: totalProjects > 0 ? `${animatedTotal}+` : '100+', label: 'Projects Completed' },
  ...
  return (
    ...
    >
      {stats.map((stat, i) => (
        <Box
          key={stat.label}
          ref={stat.label === 'Projects Completed' ? counterRef : null}
          sx={{
            display: 'flex',
            flexDirection: 'column',
            alignItems: 'center',
...
const CERTS = [
  { Icon: EmojiEventsIcon, title: 'GAF Master Elite', sub: 'Top 3% of contractors', url: 'https://www.gaf.ca/en-ca/roofing-contractors/residential/usa/ny/albany/nova-solutions-1151568' },
          ...
          {CERTS.map(({ Icon, title, sub, url }) => (
            <Box
              ...
              sx={{ 
                ...
                '&:hover': url ? { transform: 'translateY(-3px)', borderColor: gold } : {}
              }}
            >
...
export default function AboutPage() {
  // Fetch with limit 1 just to get the total gracefully
  const { totalProjects } = useLandingProjects(1);
  const { summary } = usePublicReviews({ limit: 1 });

  return (
    <Box sx={{ width: '100%', minHeight: '100vh', bgcolor: '#0D0D0D' }}>
      <SEO 
        title="About Nova Solutions Restoration | Roofing & Exterior Experts in Albany"
        description="Discover Nova Solutions Restoration, the Capital Region’s trusted provider of roofing, siding, and gutters. Over 5 years of certified excellence in home exterior restoration."
        keywords="about Nova Solutions Restoration, roofing experts Albany, siding contractors Capital Region, home exterior specialists NY, local roofers Albany"
      />
      <AboutHero />
      <StatsBar 
